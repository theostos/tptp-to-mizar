...
from .datamodule import mirza
from .datamodule_noreasoning import mirza_noreasoning
...
def configure_finetuning_recipe(config):
    ...
    if config['reasoning']:
        data = mirza(model_name, **config_datamodule)
    else:
        data = mirza_noreasoning(model_name, **config_datamodule)
    return run.Partial(
        llm.finetune,
